...
This directory contains the files that are used to install [Crunchy MongoDB for GKE][gcp-details],
which uses the MongoDB Operator, from the Google Cloud Marketplace.
...
Install [Crunchy MongoDB for GKE][gcp-details] to a Google Kubernetes Engine cluster using
...
1. Configure the installation by setting environment variables.
   ...
   1. Choose a version to install.
      ...
      export mdb_VERSION=4.5.1
      export INSTALLER_IMAGE=${IMAGE_REPOSITORY}/deployer:${mdb_VERSION}
      export OPERATOR_IMAGE=${IMAGE_REPOSITORY}:${mdb_VERSION}
      export OPERATOR_IMAGE_API=${IMAGE_REPOSITORY}/mdb-apiserver:${mdb_VERSION}
      export OPERATOR_IMAGE_EVENT=${IMAGE_REPOSITORY}/mdb-event:${mdb_VERSION}
      export OPERATOR_IMAGE_SCHEDULER=${IMAGE_REPOSITORY}/mdb-scheduler:${mdb_VERSION}
      ```

   2. Choose a namespace and name for the application.

      ```shell
      export OPERATOR_NAMESPACE=mdb OPERATOR_NAME=mdb
      ```
   ...
   4. Choose default values for new MongoDB clusters.

      ```shell
      export MONGODB_METRICS=false
      export MONGODB_SERVICE_TYPE=ClusterIP
      export MONGODB_CPU=1000 # mCPU
      export MONGODB_MEM=2 # GiB
      export MONGODB_STORAGE_CAPACITY=1 # GiB
      export MONGODB_STORAGE_CLASS=ssd
...
2. From the list of applications, select _Crunchy MongoDB Operator_ then click _Delete_.
